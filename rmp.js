...
function getTID(professor) {
  ...
  const midInitial = professor.lastIndexOf(' ') !== professor.indexOf(',') + 1;
  ...
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(
      ...
      function (response) {
        //(response);
        if (response) {
          const regex = midInitial
            ? new RegExp(lastName + '\\W?,\\W?' + firstName, 'ig')
            : new RegExp(professor, 'ig');
...
function getPopup(block, showLink) {
  ...
  $('a, button, img', domparser).contents().unwrap();
